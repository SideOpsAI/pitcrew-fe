...
If `CAPTCHA_SECRET` and `BOOKING_CAPTCHA_SECRET` are both missing, booking requests return `captcha_not_configured`.
...
The challenge token is signed with `CAPTCHA_SECRET` (or `BOOKING_CAPTCHA_SECRET`), so it works without any third-party CAPTCHA service.
